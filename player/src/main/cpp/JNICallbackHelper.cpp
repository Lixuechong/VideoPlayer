...
//    this->job = job; // job，不能跨函数，想要使用必须全局引用。
    ...
    jmd_progress = env->GetMethodID(clazz, "jni_progress", "(I)V");
}
...
void JNICallbackHelper::onError(char *msg, int thread_mode) {
    ...
    if (thread_mode == THREAD_MAIN) {
        ...
        this->env->CallVoidMethod(this->job, this->jmd_error, j_msg);// 利用反射
    }
    if (thread_mode == THREAD_CHILD) {
        // 使用子线程的JniEnv，全新的env，调用java的方法。
        JNIEnv *env_child;
...
//    LOGD("JNICallbackHelper::onProgress %lf \t %d\n", audio_time, thread_mode)
    if (thread_mode == THREAD_MAIN) {
        this->env->CallVoidMethod(this->job, this->jmd_progress, audio_time);// 利用反射
    }
    if (thread_mode == THREAD_CHILD) {
        // 使用子线程的JniEnv，全新的env，调用java的方法。
        JNIEnv *env_child;
        ...
        env_child->CallVoidMethod(this->job, this->jmd_progress, audio_time);// 利用反射
        this->vm->DetachCurrentThread();
    }
}
